...
function productCard(p){return `<article class="card"><img src="${p.image||'/assets/hero-products.png'}" alt="${p.name}"><div class="card-body"><p class="eyebrow">${p.category||''}</p><h3>${p.name}</h3><p>${p.short_description||''}</p><p class="price">${p.price||''}</p><p><span class="status">${(p.inventory??0)>0?'In Stock':'Check Availability'}</span></p><a class="btn" href="/product.html?slug=${p.slug}">View Product</a></div></article>`}
async function loadProducts(limit){const products=await getProducts();const box=document.getElementById('product-grid');if(!box)return;const list=limit?products.filter(p=>p.featured).concat(products.filter(p=>!p.featured)).slice(0,limit):products;box.innerHTML=list.map(productCard).join('')}
